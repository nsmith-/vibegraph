...
#            is not a SIMD signal — all x86-64 float math lives in xmm). The
#            width verdict reads packed `pd` arithmetic on zmm, not any zmm
#            use: a prefer-256-bit build still touches zmm in moves and
#            compares.
...
function flush() {
    ...
    printf "%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", \
        name, instrs, packed_pd, scalar_sd, xmm, ymm, zmm, neon2d, calls, arith_calls, zmm_pd
}
/^[0-9a-f]+ <.*>:[ \t]*$/ {
    flush()
    name = $0
    ...
    instrs = packed_pd = scalar_sd = xmm = ymm = zmm = neon2d = calls = arith_calls = zmm_pd = 0
    if (collecting) print "\n" $0 >> fnfile
    next
}
collecting {
...
{
    printf "vector\tinstrs\tpacked_pd\tscalar_sd\txmm\tymm\tzmm\tneon.2d\tcalls\tarith_calls\tfunction\n"
    awk -F'\t' '{ v = $3 + $8; printf "%d\t%s\n", v, $0 }' "$OUT_DIR/census.tsv" \
        | sort -t$'\t' -k1,1nr -k3,3nr | head -40 \
        | awk -F'\t' '{ printf "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%.110s\n", $1,$3,$4,$5,$6,$7,$8,$9,$10,$11,$2 }'
} | if command -v column >/dev/null; then column -t -s$'\t'; else cat; fi

echo
awk -F'\t' '
{ fns++; instrs += $2; pd += $3; sd += $4; ymm += $6; zmm += $7; n2d += $8; zmm_pd += $11
  if ($1 ~ /fill_arenas/ && $10 > 0) { lane_fa++; lane_calls += $10 } }
END {
    printf "TOTAL: %d functions, %d instructions, %d packed-pd, %d scalar-sd, %d neon-.2d lines\n", \
        fns, instrs, pd, sd, n2d
    if (zmm_pd > 0)   print "verdict: packed-pd on zmm present -> 512-bit (8x f64) vectors are being emitted"
    ...
    if (lane_fa > 0)  printf "inlining: %d fill_arenas instance(s) make %d out-of-line arithmetic calls -> lane ops are not inlined; the packed counts above sit in the callees\n", lane_fa, lane_calls
    else              print "inlining: no fill_arenas instance calls out to lane arithmetic"
}' "$OUT_DIR/census.tsv"
