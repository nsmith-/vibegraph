...
## 📤 `event-output-lhef` sprint ✅ CLOSED 2026-07-28 — merge decision is the user's (branch `event-output-lhef`, HEAD `fc11526`; full plan: `research/notes/23-event-output-lhef-plan.md`)
...
| post-E4 | **Model identity in the artifact** — close the blind spot E4 filed: `IntegrateArtifact` banked no model, so `import model sm-no_b_mass` with an identical `generate` line passed the card check and produced events from a different model than the grid was trained on. | ✅ `format_version = 3` banks a `ModelIdentity` — the `import model` label plus a SHA-256 digest over the *restricted parsed model's own serialized form*, not the UFO source files — so comments and formatting that do not survive parsing cannot refuse an artifact, and the restrict card contributes through its effect rather than its text (`UFOModel::load_with_digest` loads and identifies in one path). Digesting the model exposed a real defect: five collections reachable from `ParsedModel` were `HashMap`/`HashSet`, whose iteration order varies per instance and per process, so the digest was not reproducible and `generate` would have refused `integrate`'s artifacts at random; all five are now `BTreeMap`/`BTreeSet` (behaviour-preserving, wire format unchanged, blob needs no regeneration). `card_mismatches` reports a differing label by name, and a differing digest when the labels agree — the same-name-different-contents case a name provably cannot see. Resolution and identification share `GlobalConfig::load_ufo_with_identity`, so a digest can never describe a different model than the one loaded. Mutation-checked: deleting both comparisons makes the wrong-model run write events at **+1.95%** vs the banked σ. No artifact is committed or consumed by any test/pixi task, so none needed regenerating. Standing gates unmoved. **The compiled-program cache slot is designed, not built** (note 23) — its key `(model digest, process, compiler schema version)` is already derivable from banked fields, so no dead field was reserved. |
...
channel by channel, one grid each, and `IntegrateArtifact` gained
`channels: Vec<ChannelGrid>` (`format_version` 1 → 2; it is `3` since the post-E4
row below). err²·CPU improves 1.07×–1.68× and
unweighting efficiency 1.7×–2.9× on four of five processes measured (0.91× on
...
comparison**, the **E4 leftovers** row (streaming `-4` by two-pass replay, `lpp = 1`
event generation) and the **compiled-program cache** row with its trigger
condition. `mg-single-helicity-bench` was re-sequenced out of E2 — accept/reject
never made single-helicity evaluation the hot path, so it still has no consumer.
...
| **`uux_to_uux` residual bias** | Now a hard σ GATE, but the five-seed sweep leaves a *negative* mean against banked MG — **~0.30%** since per-channel grids landed, up from ~0.17% on the same seeds under one shared grid, and it does **not** shrink with budget (~0.25% at 4×). Still far inside the 2% tolerance; worst seed \|pull\| 2.69 against the 3.5 limit. Sharper per-channel grids stop compromising with each other and so cover the spacelike collinear tail *less* — the same region a single-rung t-channel spine under-resolves, now more visible. Evidence for the multi-rung spine, not a new defect. | `validate_sigma.rs` `probe_qcd_seed_stability`; ties to the multi-rung spine row above |
...
| **E4 leftovers** | Two small ones, recorded in note 23's close-out. (1) **Streaming `IDWTUP = -4`** by deterministic two-pass replay — the interface hook (`EventSource::restart`) is in place and contract-tested, only the strategy is missing; not needed while 100k-event runs buffer in ~42 MB. (2) **Proton-beam (`lpp = 1`) event generation** — the Drell–Yan map has no channel decomposition, so no `ChannelIntegrand`; `generate` refuses it by name. *(The third, the model import in the artifact fingerprint, was closed post-E4 — `format_version = 3` banks a `ModelIdentity` and `generate` refuses a mismatch.)* | note 23 §"Sprint close-out"; `vibegraph-cli/src/generate.rs` |
| **Compiled-program cache in the artifact** | Designed in note 23's post-E4 section, deliberately **not** built. `generate` recompiles the `AmplitudeEvaluator` rather than loading one; measured cost is **0.05–0.29 s** of setup for every process the CLI can run, against ~13 s for a 20k-event `generate`. **Trigger to build it:** setup climbing to a noticeable share of a generation run — richer diagram enumeration, or e-graph extraction becoming part of compilation. The key it matches on, `(model digest, process, compiler schema version)`, is already derivable from banked artifact fields plus a compiler-side constant, so no schema bump and no reserved field is needed. Three obstacles first: nothing in `helas::eval` implements serde (and the arena encoding needs a schema version of its own, since it changes for performance); `folded_hel` is an `OnceLock` built lazily on first `eval_m2`, so "the compiled program" is not one object and the expanded arena is the large part; and `prune_zero_helicities` carries a kinematic contract (partonic-CM momenta, beams along ±z) that must be rechecked on load or a pruned program is silently wrong under other kinematics. | note 23 §"The compiled-program cache slot"; doc comment on `artifact::IntegrateArtifact` |
