...
**Known count discrepancies (root causes):**
1. **Light fermion Yukawa = 0**: SM param_card omits `yme`, `ymmu`, `ymc` → those couplings
   are numerically zero. MadGraph prunes diagrams where any vertex coupling is zero; vibegraph
   includes all symbolically-present vertices. Concretely, diagrams with H or G0 propagators
   coupling to e, μ, c are excluded by MadGraph but generated by feyngraph.
   - e⁺e⁻→μ⁺μ⁻: vibegraph generates a/G0/H/Z; MadGraph keeps only a/Z (2 vs 4 diagrams)
   - p p→l⁺l⁻ via c c~: vibegraph adds G0 and H mediators (4 vs 2 per subprocess)
2. **Subprocess expansion**: vibegraph expands all flavor combinations independently (c c~
   → e+ mu-, s~ s → mu+ e-, etc.); MadGraph groups identical topologies into representative
   subprocess classes. The "independent" count in MadGraph matches one representative only.
3. **Fix direction**: implement coupling-constant evaluation at diagram time and prune
   zero-coupling diagrams before counting; group subprocesses by topology class.
   The `goldstoneboson=True` flag in particles.py is the correct signal for unitary-gauge
   filtering of G0/G+ (the H exclusion is purely the zero-Yukawa issue, not a gauge one).
