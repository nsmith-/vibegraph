...
| V5 | `rooting-soundness` spike: root-invariant momentum routing / Lorentz rooting / fermion-spine sign; failing all-rootings gate test first | 🟡 failing gate landed (`rooting_soundness.rs`); **locus (a) FIXED** — honest `MetricVout` current + rooting-invariant `σ_V = (−1)^(#VVV at idx 1..)` in `root_diagram::yang_mills_vvv_sign`, `NegVout` op deleted; 14/14 `validate_helas_mg` bit-exact (all flows), gate 21→18 (`ee→WW`/`gg→ttx` pass). Remaining: locus (b) fermion-spine (`ee→ττH`/`ee→μμττ`/reclassified `uux/bbx` 2→6), locus (c) benign FP tol (note 19 §V5) |
