...
| V5 | `rooting-soundness` spike: root-invariant momentum routing / Lorentz rooting / fermion-spine sign; failing all-rootings gate test first | 🟡 failing gate landed (`rooting_soundness.rs`, 21/133 fail); locus (a) refined to vector-propagator sign misattributed to VVV output (blanket sink −1 falsified — regresses gg→ttx); 3-pronged fix separable (note 19 §V5) |
