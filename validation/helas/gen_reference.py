...
def main():
    ...
    with open("reference.csv", "w") as f:
        ...
        for i, sqrt_s in enumerate(sqrt_s_values):
            for j, cos_theta in enumerate(cos_theta_values):
                # cast to float for full precision repr in CSV
                f.write(
                    f"{float(sqrt_s)!r},{float(cos_theta)!r},{float(M2_grid[i, j])!r}\n"
                )
    ...
    print(
        f"{'sqrt_s':>10}  {'cos_θ':>8}  {'HELAS M²':>18}  {'QED M²':>18}  {'ratio':>8}"
    )
    max_reldiff_qed = 0.0
    for sqrt_s in [10.0, 20.0, 30.0]:
        for cos_theta in [0.0, 0.5, -0.5]:
            ...
            print(
                f"{sqrt_s:>10.1f}  {cos_theta:>8.2f}  {helas_val:>18.8e}  "
    ...
    if max_reldiff_qed > tol_qed:
        print(
            f"WARNING: deviation {max_reldiff_qed:.2e} exceeds tolerance {tol_qed:.2e}"
        )
    else:
    ...
    for sqrt_s, cos_theta in [(MDL_MZ, 0.0), (MDL_MZ, 0.5), (MDL_MZ, -0.5)]:
        ...
        print(
            f"  √s={sqrt_s:.1f} GeV, cos_θ={cos_theta:+.1f}: "
