...
mod heavy {
    ...
    /// `t~` (`FFV2P1D_2`) and every vector-boson line get the intended form. Its
    /// `t` line is therefore never zeroed and MadEvent reads 37% above this
    /// side; with that one routine corrected, MadGraph's standalone `SMATRIX`
