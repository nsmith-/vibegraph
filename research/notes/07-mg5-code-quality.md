...
### `aloha/create_aloha.py` — Direct Bug Found (`$` never vetoes a flipped fermion line)

Found by the `process-grammar` sprint's `$` work (note 38 §4 S3 Landed; guide chapter 09,
the `$` section) and traced to its line in that sprint's close-out. Status: **not
filed**; the draft below is for the user to file.

`$ A` on a process line makes the generated amplitude multiply each marked `A` propagator
by a step function that vanishes inside the Breit–Wigner window (ALOHA's `P1D` routines,
tag `1D`). `get_custom_propa` writes that factor once, for every spin
(`create_aloha.py:527–530` at `b7687064`, 3.7.1):

```python
numerator = "theta_functionr( (P(-1,id)**2 -(Mass(id)-BWCUTOFF*Width(id))**2 ) *( P(-1,id)**2 - (Mass(id)+BWCUTOFF*Width(id))**2),1,0)"
```

For a fermion propagator whose spinor index is odd — the off-shell spinor built from a
vertex's second spinor slot, `FFV2P1D_1` — `needPflipping` is set, and
`parse_expression` (`:262–263`) flips the momentum by a regex that prefixes every `P(`
with `-`. `P(-1,id)**2` becomes `-P(-1,id)**2`, which the expression parser reads with
Python's precedence as `-(p²)`. The argument becomes
`(p² + (M − cΓ)²)(p² + (M + cΓ)²)`, which is never negative, so the step function is 1
everywhere and the line is never zeroed. The generated Fortran shows it directly:

```fortran
C FFV2P1D_1.f (the flipped slot): the argument is (p^2 + A)(p^2 + B)
      FCT2 = THETA_FUNCTIONR((TMP0*(TMP0+FCT1+FCT0)+FCT1*FCT0),1D0,0D0)
C FFV2P1D_2.f (the other slot): (p^2 - A)(p^2 - B), as intended
      FCT5 = THETA_FUNCTIONR((TMP1*(-1D0)*(-TMP1+FCT4+FCT3)+FCT4*FCT3)
```

**Scope, measured on the generated routines.** Writing the square as a product fixes it
(the one-line diff below, committed as
`validation/madgraph/patches/aloha-p1d-flipped-fermion.patch`). Generated by the pinned
ALOHA with and without it, `FFV2P1D_1` changes to exactly `FFV2P1D_2`'s form, and
`FFV2P1D_2`, `FFV2P1D_3` (vector), `VVV1P1D_1` and `FFS4P1D_3` (scalar) come out
byte-identical: only the flipped fermion routine is affected. By reading, not tested: a
UFO custom propagator that writes `P(-1,id)**2` for a fermion goes through the same regex
and would flip the same way.

**Observed effect.** On `u u~ > w+ b w- b~ $ t t~` at fixed √s = 500 GeV the `t~` line
(`FFV2P1D_2`) is zeroed on its window and the `t` line (`FFV2P1D_1`) is not. MadEvent
reads 0.06811 ± 0.00008 pb (five seeds of 10k events) where vibegraph's `|M'|²`, every
marked line zeroed, integrates to 0.04982 ± 0.00004 pb: 37% above. With the one routine
corrected in the generated `Source/DHELAS`, MadGraph's standalone `SMATRIX` equals
vibegraph's zeroed amplitude to 1e-12 at six points (both tops, either one, and neither
inside its window), and MadEvent reads 0.0494–0.0498 pb at a fixed μ = 173 GeV where
vibegraph reads 0.0506–0.0508. Regenerated end to end by the patched ALOHA itself
(`gen_onshell_veto.sh`, five seeds at the row's own card), MadEvent reads
0.04888 ± 0.00020 pb (seed χ²/dof 5.9, one seed 1.8% below the other four) against
vibegraph's 0.04982: that remaining ~2% is open and is not this defect. Every
row whose marked line is a vector boson (`e+ e- > mu+ mu- $ z`, `t > b e+ ve $ w+`,
`p p > e+ e- $ z`) agrees with vibegraph within its seed errors, as the scope predicts.

**Upstream report draft.**

> **Title:** `$` (forbidden on-shell s-channel) never vetoes a fermion propagator
> computed from a vertex's second spinor slot (ALOHA `P1D`: sign flip inside a square)
>
> **Version:** MadGraph5_aMC@NLO 3.7.1 (`aloha/create_aloha.py`; older lines not
> checked).
>
> **Summary.** For `generate u u~ > w+ b w- b~ $ t t~`, the generated
> `Source/DHELAS/FFV2P1D_1.f` (the top line) evaluates the Breit–Wigner veto as
> `THETA_FUNCTIONR((TMP0*(TMP0+FCT1+FCT0)+FCT1*FCT0),1D0,0D0)`, that is
> θ((p² + (M − cΓ)²)(p² + (M + cΓ)²)), which is 1 for every momentum, while
> `FFV2P1D_2.f` (the anti-top line) has the intended θ((p² − (M − cΓ)²)(p² − (M + cΓ)²)).
> The anti-top window is removed and the top window is not, and the cross section at
> √s = 500 GeV is 37% too large.
>
> **Cause.** `AbstractRoutineBuilder.get_custom_propa` writes the `1D` numerator with
> `P(-1,id)**2`. For an outgoing fermion with an odd spinor index `needPflipping` is set,
> and `parse_expression` rewrites `P(` to `-P(`, giving `-P(-1,id)**2`, which parses as
> `-(P(-1,id)**2)`.
>
> **Fix** (`aloha/create_aloha.py`, the `propa == "1D"` branch):
>
> ```diff
> -            numerator = "theta_functionr( (P(-1,id)**2 -(Mass(id)-BWCUTOFF*Width(id))**2 ) *( P(-1,id)**2 - (Mass(id)+BWCUTOFF*Width(id))**2),1,0)"
...
> ```
>
> With it, `FFV2P1D_1` is generated in `FFV2P1D_2`'s form and every other `P1D` routine
> (vector, scalar, even-slot fermion) is byte-identical to before. A more general fix
> would make the flip in `parse_expression` bind tighter than `**` (rewrite `P(...)` to
> `(-P(...))`), which would also cover a user-supplied custom propagator written with
> `**2`.
>
> **Reproduce** (seconds; no event generation):
>
> ```
> import model sm
> generate u u~ > w+ b w- b~ $ t t~
> output standalone uuwbwb_veto
> ```
>
> and compare the `THETA_FUNCTIONR` lines of `uuwbwb_veto/Source/DHELAS/FFV2P1D_1.f` and
> `FFV2P1D_2.f`. For the effect on a cross section, `output` and `launch` the same process
