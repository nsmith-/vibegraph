...
([`ixxxxx`](../api/vibegraph/helas/wavefn/type.InDiracWf.html) for flowing-in,
[`oxxxxx`](../api/vibegraph/helas/wavefn/type.OutDiracWf.html) for the
flowing-out conjugate $\bar u, \bar v$, both built by
[`DiracWf::from_momentum`](../api/vibegraph/helas/wavefn/struct.DiracWf.html#method.from_momentum)),
a polarisation four-vector $\epsilon^\mu(p,h)$ for a vector boson
([`vxxxxx`](../api/vibegraph/helas/wavefn/struct.VectorWf.html#method.vxxxxx)),
a bare 1 for a scalar
([`sxxxxx`](../api/vibegraph/helas/wavefn/struct.ScalarWf.html#method.sxxxxx)).
Each carries its momentum along, since the routines downstream need it.
...
their output: [`jioxxx`](../api/vibegraph/helas/vertex/fn.jioxxx.html) builds
a vector current from a fermion pair,
[`fvixxx`](../api/vibegraph/helas/vertex/fn.fvixxx.html) a fermion current from
a fermion and a vector.
...
$\gamma^5 = \mathrm{diag}(-1,-1,1,1)$; massless spinors taken as the
$m \to 0$ limit of the same helicity construction, with no reference
momentum; polarisation vectors with HELAS's signs. MadGraph's helicity labels are those of
[Buarque Franzosi et al. 2020](../bibliography.md#helicity-amplitudes). The
[`wavefn` module documentation](../api/vibegraph/helas/wavefn/index.html) is the
complete reference for what the crate implements: the explicit spinors and
polarisation vectors including the at-rest, on-axis and massless special cases,
the sign the momentum is stored with for each direction of flow, and the
normalisation and completeness relations the results satisfy.
...
`helas::repr` layer. Scalar field here is the algebraic sense of the term, the
number system the arithmetic is carried out in, which is the real numbers or a
SIMD array of them; it has nothing to do with a spin-0 quantum field. A
Lorentz vector carries a phantom type recording whether it is contravariant
or covariant, so the metric is applied exactly where an index is lowered and
nowhere else. A Dirac wavefunction carries a
