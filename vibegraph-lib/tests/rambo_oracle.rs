//! RAMBO validation: uniforms-replay oracle and a flat-MC cross-section check.
//!
//! The replay oracle (`replay_matches_python`) feeds the exact uniforms dumped
//! by `validation/rambo/dump_rambo_fixture.py` through the Rust `rambo` map and
//! matches momenta per-component and the weight — pinning the deterministic
//! `uniforms -> (momenta, weight)` map against an independent implementation.
//!
//! The flat-MC check (`flat_mc_partonic_sigma`, `#[ignore]`) integrates the
//! partonic σ̂ for `u u~ > c c~ e+ e- mu+ mu-` at √ŝ = 500 GeV with flat RAMBO
//! weights and compares against the banked MadGraph value, exercising the
//! end-to-end weight normalization. Run it with:
//!
//!     cargo test -p vibegraph-lib --features extended-validation \
//!         --test rambo_oracle -- --ignored --nocapture
