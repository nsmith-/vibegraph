...
fn gated_rows_op_census() {
    ...
    // and `EpsilonVout` (a Levi-Civita tensor rooted at a vector leg rather than
    // closing into the amplitude) exist in this model but not in these rows: the
    // dipole reaches γ⁵ only inside a chain, and `g g > h` contracts its `Epsilon`
    // into a scalar. The triple-gauge and four-gluon rows reach both, and take them
    // off this list when they gate.
